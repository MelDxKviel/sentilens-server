...
@music_router.get("/")
async def get_music_list(session: AsyncSession = Depends(get_session)) -> list[MusicRead]:
...
async def get_music_by_id(music_id: int, session: AsyncSession = Depends(get_session)) -> MusicRead:
